...
namespace AWSSDK.Examples
{
    public class AppManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            buttonUploadFileBucket.onClick.AddListener(() => { UploadObjectForBucket(pathFileUpload, S3BucketName, folderNameOnBucket + fileNameOnBucket); });
        ...
        private void GetObjectBucket(GetObjectResponse resultFinal = null, string errorFinal = null)
        {
            ...
            S3Manager.Instance.GetObjectBucket(S3BucketName, folderNameOnBucket + fileNameOnBucket, (result, error) =>
            {
                if (string.IsNullOrEmpty(error))
                {
                    resultTextOperation.text += "\nGet Data Complete.";
        ...
        {
            resultTextOperation.text = string.Format("deleting {0} from bucket {1}\n", folderNameOnBucket + fileNameOnBucket, S3BucketName);

            S3Manager.Instance.DeleteObjectOnBucket(folderNameOnBucket + fileNameOnBucket, S3BucketName, (result, error) =>
            {
                if (string.IsNullOrEmpty(error))
                {
